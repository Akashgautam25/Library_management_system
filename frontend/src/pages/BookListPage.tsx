    ...
    return (
        <div className="p-6 max-w-7xl mx-auto min-h-screen">
            ...
            <motion.div 
                ...
                className="glass-panel p-6 rounded-2xl mb-8 flex flex-col md:flex-row gap-6 items-center justify-between overflow-hidden"
            >
                <form className="flex w-full md:max-w-md relative flex-shrink-0" onSubmit={handleSearch}>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar w-full md:flex-1 md:justify-start flex-nowrap">
                    <button
                        className={`whitespace-nowrap px-4 py-2 rounded-xl text-sm font-semibold transition-all border flex-shrink-0 ${!selectedCategory ? 'bg-accent text-white border-accent shadow-lg shadow-accent/20' : 'bg-surface hover:bg-white/5 text-secondaryText border-white/10 hover:text-white'}`}
                    ...
                    {BOOK_CATEGORIES.map((cat) => (
                        <button
                            key={cat}
                            className={`whitespace-nowrap px-4 py-2 rounded-xl text-sm font-semibold transition-all border flex-shrink-0 ${selectedCategory === cat ? 'bg-accent text-white border-accent shadow-lg shadow-accent/20' : 'bg-surface hover:bg-white/5 text-secondaryText border-white/10 hover:text-white'}`}
